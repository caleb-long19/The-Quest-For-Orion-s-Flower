...
public class Dialog : MonoBehaviour
{
    ...
    private void Update()
    {
        if(DialogBoxMum.activeInHierarchy && Input.GetKeyDown(KeyCode.Space)) // Press Space to close Mum's Dialog Box
        {
            DialogBoxMum.SetActive(false);
        }
        else if (DialogBoxDad.activeInHierarchy && Input.GetKeyDown(KeyCode.Space)) // Press Space to close Dad's Dialog Box
        {
            DialogBoxDad.SetActive(false);
        }
